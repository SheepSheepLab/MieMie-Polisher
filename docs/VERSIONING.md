...
本次 Polisher 版本为 `1.1.1`，仅替换 SheepSheep 提供的正式 Icon 并提升 PATCH，不修改业务或数据结构。GitHub 分发文件为 ASCII `MieMie-Polisher-Extension-1.1.1.json`、`manifest.json` 与机器元数据 `MieMie-Extension-update.json`；本地同时生成字节一致的中文产品文件名。1.1.0 及更早历史发布保持不变。
